...
const viewDetails = async (id, _id) => {
  ...
  for (let i = 0; i < gasolineChainsawArray.value.length; i++) {
    dataInstrument.value = _.filter(gasolineChainsawArray.value, {"_id": _id})
  }
...
const buyInBasket = async (id, _id) => {
  ...
  if (counterClick.value === 1) {
    ...
    localStorage.setItem("id_gasoline", JSON.stringify(id))
    await router.push({name: 'gasolineInstrumentChainsawId', params: {id: id}})
...
onMounted(() => {
  if (gasolineChainsawArray.value[0].availability === 0) {
