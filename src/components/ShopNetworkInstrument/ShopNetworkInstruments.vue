...
const networkInstrumentsFunc = async (network, typeThisSecond, trueOrFalse) => {
  try {
    if (trueOrFalse) {
      let filterArray = ref(null)
      filterArray.value = _.filter(network, {typeThis: typeThisSecond})
      networkArray.value = filterArray.value
    } else {
      for (let i = 0; i < network.length; i++) {
        networkArray.value.push(network[i])
      }
    }
    return true
  } catch (err) {
...
const visibleItems = computed(() => networkArray.value.slice(0, instrumentShow.value))
const allItemsShown = computed(() => instrumentShow.value >= networkArray.value.length)
