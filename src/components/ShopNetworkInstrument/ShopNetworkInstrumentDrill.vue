...
//
const networkDrillArray = []
const networkDrill = () => {
  for (let i = 0; i < networkLocal.length; i++) {
    if (networkLocal[i].typeThis === 'Сетевая дрель') {
      networkDrillArray.push(networkLocal[i])
...
const viewDetails = (id) => {
  router.push({name: 'networkInstrumentDrillId', params: {id: id}}) // /id/:id
...
const buyInBasket = (id) => {
  ...
  if (counterClick.value === 1) {
    ...
    localStorage.setItem("id_network", JSON.stringify(id))
    router.push({name: 'networkInstrumentDrillId', params: {id: id}})
  }
}
  ...
  <v-card
    ...
    <v-row>
      ...
      <v-col :cols="secondColFunc()"
        ...
        <div
            class="mt-1">
          <v-card-text v-for="item in networkDrillArray[i.numberInList].featureTopTitle"
                       key="item"
