...
const pneumoNailGun = async (pneumo) => {
  for (let i = 0; i < pneumo.length; i++) {
    if (pneumo[i].typeThis === 'Гвоздезабивной пистолет пневматический') {
      pneumoNailGunArray.value.push(pneumo[i])
    }
  }
}
pneumoNailGun(pneumoLocal.value)
...
onMounted(() => {
  if (pneumoNailGunArray.value[0].availability === 0) {
