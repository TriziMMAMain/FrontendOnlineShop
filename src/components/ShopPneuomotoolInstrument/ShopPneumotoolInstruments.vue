...
onMounted(async () => {
  ...
  if (currentUrl.value === '/pneumotool-instrument/catalog/') {
    ...
    await pneumotoolInstrumentsFunc(pneumotoolLocal.value, "Компрессор", true)
  } else if (currentUrl.value === '/pneumotool-instrument/jackhammer/') {
    await pneumotoolInstrumentsFunc(pneumotoolLocal.value, "Пневматическая отбойная молотковая машина", true)
  } else if (currentUrl.value === '/pneumotool-instrument/nail-gun/') {
    await pneumotoolInstrumentsFunc(pneumotoolLocal.value, "Гвоздезабивной пистолет пневматический", true)
  }

  if (pneumotoolArray.value.length === 0) {
