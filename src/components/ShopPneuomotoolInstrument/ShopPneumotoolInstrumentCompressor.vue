...
const pneumoCompressor = async (pneumo) => {
  for (let i = 0; i < pneumo.length; i++) {
    if (pneumo[i].typeThis === 'Компрессор') {
      pneumoCompressorArray.value.push(pneumo[i])
    }
  }
}
pneumoCompressor(pneumoLocal.value)
...
onMounted(() => {
  if (pneumoCompressorArray.value[0].availability === 0) {
